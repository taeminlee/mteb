...
def bm25kor_loader(**kwargs):
    ...
    class BM25KorSearch(DRESModel):
        ...
        def encode(self, texts: list[str], **kwargs):
            ...
            if isinstance(texts, str):
                texts = ' '.join([token.form for token in self.kiwi.tokenize(texts)])
            else:
                texts = [' '.join([token.form for token in self.kiwi.tokenize(text)]) for text in texts]
            return bm25s.tokenize(texts, stopwords=None, stemmer=None)
    ...
    return BM25KorSearch(**kwargs)


bm25_s_kor = ModelMeta(
    loader=partial(bm25kor_loader, model_name="bm25s-kor"),  # type: ignore
    name="bm25s-kor",
    languages=["kor_Hang"],
    open_source=True,
    revision="0_1_11",
    release_date="2024-09-03",  ## release of version 0.1.11
)
